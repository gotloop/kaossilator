...
class KaossModel {
  ...
  setFrequency(pitch) {
    ...
    this.currentOffset = this.scale[
      Math.floor(this.currentPitch * this.scale.length + 1)
    ];
    this.oscillatorNode.frequency.value = this.getFrequency();
  }
  getFrequency() {
...
class TouchPadView {
  ...
  onMouseMove(event) {
    ...
    let pressed = this.convertCoordinates(
      event.offsetX,
      event.offsetY,
      this.element.width,
      this.element.height,
...
class SelectComponent {
  ...
  renderValue(option) {
    ...
    a.setAttribute(
      "onclick",
      `javascript:document.forms[0]['${this.element.id}'].value =  '${option.value}'`,
    );
...
class OptionsView {
  constructor(private model: any, private element: HTMLElement) {
    ...
    element.addEventListener("change", (event: Event) => {
      ...
      switch (event?.target?.id) {
        case "type":
          ...
          break;
        case "tonic":
          this.model.setTonic(event.target.value);
          break;
        case "octave":
          this.model.setOctave(event.target.value);
          break;
        case "mode":
          this.model.setMode(event.target.value);
          break;
      }
      console.log(event);
      console.log(this.model);
    });
    for (let option of element.getElementsByTagName("select")) {
      new SelectComponent(option);
    }
  }
}
//runtime
